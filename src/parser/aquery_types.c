...
{
	return x == BOOLEAN_TYPE;
}

int is_numeric(DataType x)
{
	return is_int(x) || is_float(x) || is_bool(x);
}
...
{
	//we know results have to be x type, only label as error if neither operand had errors
	//and we unified with error rather than unknown
	return (is_error(poss) && !is_error(x) && !is_error(y)) ? ERROR_TYPE : override;
}
