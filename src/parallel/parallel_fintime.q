...
.qpar.q2.query:{
  ...
  // distributedly join price and split data
  .aq.par.master.join[ej;`Id`TradeDate;`pxdata;`splitdata;`joined];
...
.qpar.q5.query:{
  ...
  // grouping by Id means all instances of Id=x are in a given process
  ...
  // note that join results in partitioning by Id+TradeDate, so there might
  // be instances of Id=x scattered across different processes, which means
  // we need to actually perform group
  .aq.par.master.groupby[`joined;{`Id xgroup x};`joined];

  // sort columns in grouped table
  // note that while distributed group-by guarantees that consecutive groups are stored
  // in consecutive processes (i.e. if groups are 0...10, and 2 processes, groups 0..5 are in 
  // proc 1 and remainig in proc 2), there is no guarantees for the order of the groups within
  // each process (i.e guarantees are inter-process, not intra-process)
  // so we sort by Id in each process
  {
...
.qpar.q6.query:{
  ...
  raze { select Id, CrossDate:TradeDate, ClosePrice from ungroup joined where CrossPoints } peach .z.pd[]
  };
...
.qpar.q7.query:{
  ...
  // we've grouped by Id, so all observations for a given Id are in the same process
  {
    `splitadj set 0!select first ClosePrice* prd SplitFactor by Id, TradeDate from splitadj 
      where TradeDate < SplitDate
  } peach .z.pd[];
  ...
  .aq.par.master.groupby[`adjpxdata;{`Id xgroup x};`adjpxdata];
  { 
  ...
  // adjpxdata is also grouped by Id (as is simualted, which stems from adjpxdata)
